...
const getUserAnalytics = async (req, res) => {
  try {
    ...
    const previousPeriodStart = new Date(dateFilter[Op.gte].getTime() - (now.getTime() - dateFilter[Op.gte].getTime()));
...
const getJobAnalytics = async (req, res) => {
  try {
    ...
    const jobTrends = await Job.findAll({
      ...
      order: [[Job.sequelize.fn('DATE', Job.sequelize.col('created_at')), 'ASC']],
      raw: true
    });
...
const getApplicationAnalytics = async (req, res) => {
  try {
    ...
    const newApplications = await Application.count({ where: { applied_at: dateFilter } });

    // Calculate rejection rate
    const reviewedApplications = acceptedApplications + rejectedApplications;
    const rejectionRate = reviewedApplications > 0 
    ...
    if (reviewedApps.length > 0) {
      const totalResponseTime = reviewedApps.reduce((sum, app) => {
        const responseTime = new Date(app.reviewed_at) - new Date(app.applied_at);
    ...
    const applicationTrends = await Application.findAll({
      attributes: [
        [Application.sequelize.fn('DATE', Application.sequelize.col('applied_at')), 'date'],
        [Application.sequelize.fn('COUNT', Application.sequelize.col('applications_id')), 'count']
      ],
      where: { applied_at: dateFilter },
      group: [Application.sequelize.fn('DATE', Application.sequelize.col('applied_at'))],
      order: [[Application.sequelize.fn('DATE', Application.sequelize.col('applied_at')), 'ASC']],
      raw: true
    });
